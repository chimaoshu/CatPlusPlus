...
private:
    ...
    void read_kv_config(const std::string &path)
    {
        ...
        while (std::getline(file, line))
        {
            ...
            std::string value = UtilString::strip(line.substr(spliter + 1, line.size() - spliter - 1));
            ...
            if (!key.empty() && value.empty())
                UtilError::error_exit("line " + std::to_string(line_num) + " has key but no value", false);

            // 重复键
            if (config_.find(key) != config_.end())
                UtilError::error_exit("duplicate key \"" + key + "\" in line " + std::to_string(line_num), false);
...
public:
    static std::string get(std::string key, const std::string &default_value = "")
    {
