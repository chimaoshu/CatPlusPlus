...
// 格式化为日志格式
void Logger::format_str(const std::string &type, const std::string &msg, std::string &result)
{
    // 获取时间
    time_t now = time(0);
...
void Logger::io_uring_prep_submit_task(const LogTask &task)
{
    ...
    if (task.buffer_info->buf_index < max_fixed_buffer_capacity_)
        io_uring_prep_write_fixed(sqe, task.file_info->file_index, task.buffer_info->iov.iov_base, task.bytes_len, 0, task.buffer_info->buf_index);
    // 极少数情况，当可注册的buffer pool扩展达到上限时，使用不注册的buffer提交
    else
        io_uring_prep_write(sqe, task.file_info->file_index, task.buffer_info->iov.iov_base, task.bytes_len, 0);
        ...
        else
        {
            ...
            while (!unused_buffer_.pop(buffer_info))
            {
                ...
                // 其他错误
                    ...
                    UtilError::error_exit("io_uring_wait_cqe_nr failed with " + std::to_string(ret) + " while waiting for buffer", false);
            }
            unused_buffer_num_--;
        }
    }
...
void Logger::log(const std::string &logname, const std::string &msg)
{
    ...
    if (unsubmitted_tasks_num_ + submitted_unconsumed_task_num_ >= unused_buffer_num_)
    {
        // 抢到sq锁的线程向io_uring提交IO写请求，抢不到锁不阻塞
        if (std::unique_lock sq_lock(io_uring_sq_mutex_, std::try_to_lock); sq_lock.owns_lock())
        {
            // 填充sqe
            LogTask task;
            while (unsubmitted_tasks_.pop(task))
        ...
        if (std::unique_lock cq_lock(io_uring_cq_mutex_, std::try_to_lock); cq_lock.owns_lock())
        {
            struct io_uring_cqe *cqe;
            // 检查是否有新cqe，抢不到锁不阻塞
