...
private:
  ...
  struct FileInfo
  {
    ...
    FileInfo() = default;
    FileInfo(int fd, int registered_index)
        : fd(fd), file_index(registered_index) {}
  };
...
private:
  ...
  // 格式化为日志格式
  void format_str(const std::string &type, const std::string &msg,
                  std::string &result);
...
public:
  ...
  // @param logname_list 日志名数组, e.g. std::vector<string>{"debug", "info",
  // "warn", "error"}
  // @param init_fixed_buffer_num fixed buffer初始数量
  // @param max_fixed_buffer_pool_size fixed
  // buffer不足时会扩展，该字段为其数量上限
  // @param io_uring_entries sqe大小，不能少于max_fixed_buffer_pool_size
  Logger(const std::string &log_dir, const std::vector<LogName> &logname_list,
         int init_fixed_buffer_num, int max_fixed_buffer_pool_size,
