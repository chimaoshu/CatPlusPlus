...
ConnectionTask handle_http_request(int sock_fd_idx, ProcessFuncType processor)
{
  while (true)
  {
    ...
    if (!bad_request && args.use_web_server)
    {
      ...
      // 先打开文件，后续如果是splice模式, 则需要先序列化并发送header，然后body使用splice一边读取一边写入socket
      if (use_direct_file)
      {
    ...
    // web file size大于阈值才使用splice进行发送
    static int splice_threshold = config::force_get_int("SPLICE_THRESHOLD");
    bool use_splice_to_transfer_body = !bad_request && ((web_server_file_fd != -1) &&
    ...
    if (!bad_request && args.use_web_server && !use_splice_to_transfer_body)
    {
      void *buf = NULL;
      int used_buf_id = -1;
      ...
      if (read_success)
      {
        read_file_used_buf[buf] = used_buf_id;
      ...
      else if (!read_success)
      {
        ...
        response_buf.version(request.version());
        response_buf.result(http::status::not_found);
        response_buf.keep_alive(request.keep_alive());

        response_buf.body().data = NULL;
        response_buf.body().size = 0;
        response_buf.body().more = false;
    ...
    if (args.use_web_server && !use_splice_to_transfer_body)
    {
      serializer_buf = new http::response_serializer<http::buffer_body>(response_buf);
      serialize(serializer_buf, buffers, ec, data_to_consume, false);
    }
    else
    {
      serializer = new http::response_serializer<http::string_body>(response);
      serialize(serializer, buffers, ec, data_to_consume, true);
    }
    if (ec)
    {
    ...
    auto awaitable_send = socket_send(sock_fd_idx, buffers, send_error_occurs, read_file_used_buf);
    while (!finish_send)
    {
    ...
    if (args.use_web_server && !use_splice_to_transfer_body)
    {
      ...
      for (auto it : read_file_used_buf)
      {
        if (it.second == -1)
    ...
    if (send_error_occurs ||
        !request.keep_alive() ||
        (serializer && !response.keep_alive()) ||
        (serializer_buf && !response_buf.keep_alive()))
    {
